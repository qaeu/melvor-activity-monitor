...
export class CompressionUtil {
    ...
    static async compress(data) {
        if (!this.isSupported()) {
            logger.warn('CompressionStream not supported - storing uncompressed');
        ...
        try {
            ...
            logger.debug(`Compressed ${json.length} bytes → ${compressed.length} bytes (${ratio}% reduction)`);
            return {
                compressed,
                uncompressedSize: json.length,
                version: this.VERSION
    ...
    static async decompress(store) {
        if (!this.isSupported()) {
            logger.warn('DecompressionStream not supported - reading uncompressed');
        ...
        try {
            ...
            logger.debug(`Decompressed ${store.compressed.length} bytes → ${text.length} bytes`);
            return result;
        }
        catch (error) {
