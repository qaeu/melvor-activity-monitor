...
async function loadHauntedLibraries(ctx, logger) {
    ...
    try {
        ...
        if (window.litHtml && window.haunted) {
            ...
            logger.error("Failed to load Haunted libraries - missing global objects");
            return false;
        }
    }
    catch (error) {
...
export async function setup(ctx) {
    ...
    ctx.onCharacterLoaded(async () => {
        try {
            ...
            logger.info(`Loaded notifications - Count: ${stats.count}, Unread: ${stats.unreadCount}`);
            logger.info(`Storage stats - Compressed: ${(stats.compressedSize / 1024).toFixed(2)}KB, Ratio: ${stats.compressionRatio.toFixed(1)}%`);
        ...
        if (config.debug) {
            ctx.onInterfaceReady(() => {
                setTimeout(() => {
                    ...
                    globalThis.game.notifications.createSuccessNotification('Activity Monitor is now capturing notifications!');
                }, 2000);
            });
        }
    }
    ...
    if (!hauntedAvailable) {
        logger.error('Haunted is required for this mod. Please ensure Haunted libraries are available.');
    ...
    logger.info('Global API: ActivityMonitorMod.getStats(), ActivityMonitorMod.getNotifications()');
}
