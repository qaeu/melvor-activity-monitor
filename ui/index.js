...
function patchMinibar(ctx) {
    ctx
        .patch(Minibar, "initialize")
        .replace(function (originalFunction) {
        const result = originalFunction.call(this);
        const iconUrl = ctx.getResourceUrl("assets/icon.png");
        const minibarItem = game.minibar.createMinibarItem("minibar-ActivityMonitor", iconUrl, "Activity Monitor - Click to toggle panel", {
            onClick: () => {
                togglePanel();
            },
        });
...
function setupNotificationListeners() {
    ...
    document.addEventListener('activity-monitor-notification-added', ((event) => {
