...
function StorageModeSetting() {
	...
	useEffect(() => {
		...
		if (settings) {
			setStorageMode(
				settings.getSetting('storageMode') || 'local-storage',
			);
			setCharacterSaveType(
				settings.getSetting('characterSaveType') || 'percentage',
			);
			setCharacterSavePercentage(
				settings.getSetting('characterSavePercentage') || 20,
			);
			setCharacterSaveLineCount(
				settings.getSetting('characterSaveLineCount') || 100,
			);
			setLocalStorageLineCount(
				settings.getSetting('localStorageLineCount') || 500,
			);
		}
	}, []);
	...
	return html`
		<div class="mb-4">
			...
			<small class="form-text text-muted d-block mb-2"
				>Where to store notification history</small
			>
			<select
				class="form-control"
				.value=${storageMode}
			...
			>
				...
				<option value="character-save">
					Character Save (Cloud Sync, 8KB limit)
				</option>
				<option value="memory-only">
		...
		${storageMode === 'character-save' ?
			html`
				<div class="mb-4">
					...
					<small class="form-text text-muted d-block mb-2"
						>How to limit notifications (only for Character Save
						mode)</small
					>
					<select
						class="form-control"
						.value=${characterSaveType}
						@change=${handleCharacterSaveTypeChange}
					>
						<option value="percentage">
							Percentage of 8KB Max
						</option>
					...
					html`
						<div class="mb-4">
							<label class="form-label"
								>Percentage of Storage</label
							>
							<small class="form-text text-muted d-block mb-2"
								>Percentage of 8KB to use (1-100%). 20% ≈ 25-30
								notifications</small
							>
							<input
								type="number"
				...
				:	html`
						<div class="mb-4">
							...
							<small class="form-text text-muted d-block mb-2"
								>Maximum notifications to store</small
							>
							<select
								class="form-control"
								.value=${characterSaveLineCount.toString()}
								@change=${handleCharacterSaveLineCountChange}
							>
		...
		${storageMode === 'local-storage' ?
			html`
				<div class="mb-4">
					...
					<small class="form-text text-muted d-block mb-2"
						>Maximum notifications to store (only for Local Storage
						mode)</small
					>
					<select
						class="form-control"
						.value=${localStorageLineCount.toString()}
						@change=${handleLocalStorageLineCountChange}
					>
						<option value="100">100 notifications</option>
						<option value="250">250 notifications</option>
						<option value="500">500 notifications</option>
...
customElements.define(
	...
	component(StorageModeSetting, { useShadowDOM: false }),
);
