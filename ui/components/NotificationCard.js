...
function NotificationCard(props) {
    ...
    const handleDelete = () => {
        ...
        document.dispatchEvent(new CustomEvent('activity-monitor-refresh-panel'));
    };
    ...
    const getTypeClass = () => {
        ...
        if (type.includes('currency') || type.includes('gp') || type.includes('coin'))
            return 'notification-currency';
        return 'notification-default';
    };
    // Format quantity
    const formatQuantity = () => {
        if (notification.quantity === undefined || notification.quantity === null) {
            return '';
        }
    ...
    <div class="activity-monitor-card ${typeClass}">
      <div class="card-content">
        <!-- Icon -->
        ${notification.media ? html `
          <div class="card-icon">
            <img src="${notification.media}" alt="" />
            ${count > 1 ? html `<span class="card-count-badge">×${count}</span>` : ''}
          </div>
        ` : ''}
        ...
        <div class="card-body">
          ...
          <div class="card-meta">
            ...
            ${quantity ? html `<span class="card-quantity">${quantity}</span>` : ''}
...
customElements.define('activity-monitor-card', component(NotificationCard, { useShadowDOM: false }));
