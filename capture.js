...
export class NotificationCapture {
    ...
    setupSettingsListeners() {
        document.addEventListener('activity-monitor-capture-setting-changed', ((event) => {
    ...
    setupPatches(ctx) {
        ...
        ctx.patch(NotificationsManager, 'createErrorNotification').after((result, customID, msg) => {
        ...
        ctx.patch(NotificationsManager, 'createSuccessNotification').after((result, customID, msg, media, quantity = 1) => {
        ...
        ctx.patch(NotificationsManager, 'createInfoNotification').after((result, customID, msg, media) => {
        ...
        ctx.patch(NotificationsManager, 'createItemNotification').after((result, item, quantity) => {
            const notifType = quantity > 0 ? 'AddItem' : 'RemoveItem';
            const settingKey = quantity > 0 ? 'captureItemsAdded' : 'captureItemsRemoved';
        ...
        // quantity > 0 = add, quantity < 0 = remove
        ctx.patch(NotificationsManager, 'createCurrencyNotification').after((result, currency, quantity) => {
            ...
            if (currency.id === 'melvorD:GP') {
                ...
                settingKey = isAdd ? 'captureSlayerCoinsAdded' : 'captureSlayerCoinsRemoved';
            }
            else {
                notifType = isAdd ? 'AddCurrency' : 'RemoveCurrency';
                settingKey = isAdd ? 'captureCurrencyAdded' : 'captureCurrencyRemoved';
            }
            if (this.shouldCapture(notifType, settingKey)) {
                this.captureNotification({
                    type: notifType,
        ...
        // quantity > 0 = add, quantity < 0 = remove
        ctx.patch(NotificationsManager, 'createGPNotification').after((result, quantity) => {
            const notifType = quantity > 0 ? 'AddGP' : 'RemoveGP';
            const settingKey = quantity > 0 ? 'captureGPAdded' : 'captureGPRemoved';
            if (this.shouldCapture(notifType, settingKey)) {
                this.captureNotification({
                    type: notifType,
        ...
        // quantity > 0 = add, quantity < 0 = remove
        ctx.patch(NotificationsManager, 'createSlayerCoinsNotification').after((result, quantity) => {
            const notifType = quantity > 0 ? 'AddSlayerCoins' : 'RemoveSlayerCoins';
            const settingKey = quantity > 0 ? 'captureSlayerCoinsAdded' : 'captureSlayerCoinsRemoved';
            if (this.shouldCapture(notifType, settingKey)) {
                this.captureNotification({
                    type: notifType,
        ...
        ctx.patch(NotificationsManager, 'createSkillXPNotification').after((result, skill, xp) => {
            if (this.shouldCapture('SkillXP')) {
                this.captureNotification({
                    type: 'SkillXP',
                    message: `+${xp} ${skill.name} XP`,
                    media: skill.media,
                    quantity: xp,
                    sourceObject: skill,
                    sourceType: 'skill',
        ...
        ctx.patch(NotificationsManager, 'createAbyssalXPNotification').after((result, skill, xp) => {
            if (this.shouldCapture('AbyssalXP')) {
                this.captureNotification({
                    type: 'AbyssalXP',
                    message: `+${xp} ${skill.name} Abyssal XP`,
                    media: skill.media,
                    quantity: xp,
                    sourceObject: skill,
                    sourceType: 'skill',
        ...
        ctx.patch(NotificationsManager, 'createMasteryLevelNotification').after((result, action, level) => {
        ...
        ctx.patch(NotificationsManager, 'createSummoningMarkNotification').after((result, mark) => {
        ...
        logger.info(`Notification capture patches installed (${this.captureCount} total)`);
    }
    /**
    ...
    captureNotification(data) {
        ...
        const mediaRef = data.sourceObject && data.sourceType
        ...
        logger.debug(`Captured ${notification.type} notification:`, notification.message);
    }
    /**
     * Validate notification data before capturing
     */
    isValidNotification(data) {
        ...
        if (data.type && (data.type.includes('Item') ||
